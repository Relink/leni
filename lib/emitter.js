...
emitter.initializeProducer = function initializeProducer(client) {
  // type check client????
...
emitter.createStream = function createStream (zookeepers, clientId, consumerOpts){
  var stream, client;

  return Promise
    .try(() => {
      client = new kafka.Client(zookeepers, clientId);
      return client;
    })
    .then(client => emitter.initializeProducer(client))
    .then(producer => {
      consumer = new kafka.Consumer(client, consumerOpts.payloads, consumerOpts.options);
      stream = new ConsumerStream(consumer);
      ...
      stream.producer.on('error', err => stream.emit('error', err));
      stream.client.on('error', err => stream.emit('error', err));
      return stream;
    });
  // stream.sendMessage??
